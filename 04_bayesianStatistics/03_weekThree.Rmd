...
* <mark>There is a 95% chance that babies on average weigh 7.4 to 7.5 pounds.</mark>
</div>
...
weight_post = bayes_inference(y = weight, data = nc_fullterm, 
                              statistic = "mean", type = "ci",
                              cred_level = 0.95, show_plot = F, mu_0 = 7.7)
```
...
* <mark>there is a 95% chance that their birth weight will be 5.4 to 9.5 pounds.</mark>
...
premie_weight <- bayes_inference(y = weight, data = nc_premature, 
                        statistic = "mean", type = "ci",  
                        prior_family = "JZS", mu_0 = 7.7, rscale = 1,
                        method = "simulation",
                        cred_level = 0.95)
...
```{r HT Test for Premie}
samples = as.data.frame(premie_weight$samples)
nsim = nrow(samples)
samples = mutate(samples, y_pred = rnorm(nsim, mu, sqrt(sig2)))
...
* <mark>Both distributions are approximately symmetric.</mark>
...
nc %>% 
        group_by(gender) %>% 
        summarise(sample = n(), weight_low = min(weight), weight_high = max(weight), range = (weight_high - weight_low))
...
* <mark>there is a 95% chance that babies born to nonsmoker mothers are on average 0.11 to 0.54 pounds heavier at birth than babies born to smoker mothers.</mark>
</div>

```{r Q10-ci-weight-habit-increase}
bayes_inference(y = weight, x = habit, data = nc_fullterm,
                statistic = 'mean', type = 'ci',
                mu_0 = 0, prior = 'JZS', rscale = 1,
                method = 'simulation', show_plot = F)
```
...
```{r}
pss_both <- PrisonStress %>% 
        mutate(diff = PSSbefore - PSSafter)

bayes_inference(y = diff, data = pss_both, type = 'ht',
                statistic = 'mean', prior_family = 'JZS',
                null = 0, alternative = 'twosided', method = 'simulation',
                show_plot = F)
```
