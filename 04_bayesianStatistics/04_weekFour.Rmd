...
* Observational study - <b>Select</b>
* Experiment
</div>
...
* 10 of the respondents make strictly less than 300 dollars per week - <b> Select </b>
...
* Each additional year of education increases weekly wages by $146.95. <b> Select </b>
...
```{r Q-SLR}
education_model = lm(wage ~ educ, data = wage)

tidy(education_model)
...
* The residuals are strongly left skewed, hence the normal distribution of errors condition is not met. <b>Select</b>
...
```{r Q-resid}
ggplot(m_wage_iq_aug, aes(x = .resid)) +
        geom_histogram(binwidth = 100)
...
```{r E2-educ-resid}
m_wage_edu_append = augment(education_model)

ggplot(m_wage_edu_append, aes(x = .fitted, y = .resid)) +
        geom_point(alpha = 0.6) +
...
```{r E1-two-vars-eda}
experience_model <- lm(wage ~ exper, data = wage)

ggplot(experience_model, aes(x = .fitted, y = .resid)) +
        geom_point(alpha = 0.6) +
        geom_hline(yintercept = 0)

ggplot(experience_model, aes(x = .resid)) +
        geom_histogram() +
        xlab("Residuals")
```
...
* Case 616 has a probably of close to 1 that it an outlier under the normal error model for regressing `lwage` on `iq` <b> Select </b>
...
outliers_df2 <- data.frame(probability = outliers$prob.outlier,
                          case = 1:length(outliers$prob.outlier))

ggplot(outliers_df2, aes(ymax = probability, x = case)) +
  geom_linerange(ymin = 0) +
  labs(y = "Probability")
...
* The married black man <b> Select </b>
...
```{r E4-step}
training_model <- lm(wage ~ . -wage, data = na.omit(wage))

traing_model_stepwise <- stepAIC(training_model)
...
```{r E5-graph}
red_cofficients <- coefficients(bma_lwage_red)

plot(red_cofficients, subset = 8, ask = F)
```
...
* `married`, `age`, `black`
* `black`, `south`, `married`
* `meduc`, `urban`, `married` <b>Select</b>
</div>
```{r Q11}
variable.names(HPM_pred_lwage)
```
...
```{r Q12-cred-interval}
HPM_pred_lwage <- predict(bma_lwage, estimator = "BPM", se.fit = T)

ci_hwa_lwage <- confint(HPM_pred_lwage, estimator = "BPM")

opt_hpm <- which.max(HPM_pred_lwage$fit)

exp(ci_hwa_lwage[opt_hpm, ])
```
